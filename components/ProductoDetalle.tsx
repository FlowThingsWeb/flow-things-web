 ...
 * Ficha de producto.
 *
 * Recibe el producto y la configuración ya cargados desde el servidor
 * (app/productos/[slug]/page.tsx). Antes los pedía en un useEffect: el HTML
 * salía vacío y Google, que rastrea sin ejecutar JavaScript en la primera
 * pasada, veía una página sin descripción, sin precio y sin botón de comprar.
 * Ésa era la causa de las fichas en "Rastreada: actualmente sin indexar".
 *
 * Sigue siendo un componente de cliente —galería, variantes, carrito—, pero al
 * recibir los datos por props se renderiza también en el servidor.
 */
...
export default function ProductoDetalle({
  ...
  // Variante inicial: la del ?variante= si vino y es válida. Si no, la primera
  // con stock — sin ninguna seleccionada la ficha queda sin imagen, porque los
  // productos que cargan las fotos en las variantes no tienen imagen propia.
  const [varianteId, setVarianteId] = useState<string | null>(() => {
    const activas = ((producto.variantes || []) as Variante[]).filter(v => v.activo)
    if (activas.length === 0) return null
    if (varianteParam) {
      const pedida = activas.find(v => v.id === varianteParam)
      if (pedida) return pedida.id
    }
    return (activas.find(v => v.stock > 0) ?? activas[0]).id
  })
  ...
  // Effective stock
  const stockEfectivo = varianteSeleccionada?.stock ?? (tieneVariantes ? 0 : producto.stock)
